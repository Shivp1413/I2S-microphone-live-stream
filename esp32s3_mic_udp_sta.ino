...
// Update these with your router's credentials
const char* WIFI_SSID     = "YourRouterSSID";
const char* WIFI_PASSWORD = "YourRouterPassword";
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println("\n--- ESP32-S3 INMP441 UDP Realtime Mic (Station Mode) ---");
  ...
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("\nFATAL: WiFi connection failed. Check SSID/Password. Halting.");
    while (true) delay(1000);
  }
  ...
  udp.begin(UDP_PORT);
  Serial.printf("UDP listening on port %d. Any device on this network can now connect.\n", UDP_PORT);
